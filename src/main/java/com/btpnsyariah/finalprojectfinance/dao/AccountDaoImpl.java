...
public class AccountDaoImpl implements AccountDao{
  ...
  public Collection<FinancingAccount> getCustomerSchedule(int customerId) {
    return (Collection<FinancingAccount>)this.getSessionFactory().createQuery("select fa from FinancingAccount fa join fa.schedules sch where sch.financingAccount.id= :ScheduleId").setParameter("ScheduleId", customerId).list();
  }
}
